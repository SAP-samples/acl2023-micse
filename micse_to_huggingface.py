...
"""
Convert miCSE checkpoints to Huggingface style.
"""
...
def main():
    ...
    parser.add_argument("--path", type=str, help="Path of miCSE checkpoint folder")
    args = parser.parse_args()

    print("miCSE checkpoint -> Huggingface checkpoint for {}".format(args.path))

    state_dict = torch.load(os.path.join(args.path, "pytorch_model.bin"), map_location=torch.device("cpu"))
    new_state_dict = {}
    for key, param in state_dict.items():
        # miCSE BEGIN: SimCSE legacy code
        # Replace "mlp" to "pooler"
        # if "mlp" in key:
        #    key = key.replace("mlp", "pooler")
         # miCSE END: SimCSE legacy code
