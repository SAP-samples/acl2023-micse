...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        # check for NaN
        if (stsb_spearman != stsb_spearman) or (sickr_spearman != sickr_spearman):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if metrics is not None and self.args.metric_for_best_model is not None:
            ...
            if (
                ...
                print("--->>> New Best Model: "+str(metric_value) + " vs " + str(self.state.best_metric))
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                if (self.args.number_of_steps and self.state.global_step > self.args.number_of_steps * self.args.gradient_accumulation_steps):
                    
                    if self.state.global_step == self.args.number_of_steps * self.args.gradient_accumulation_steps:
                    ...
                    break
                #print(str(self.args.number_of_steps * self.args.gradient_accumulation_steps) + " vs " + str(step))
                ...
                if ((step + 1) % self.args.gradient_accumulation_steps != 0) and self.args.local_rank != -1:
                    ...
                    with model.no_sync():
                        try:
                            ...
                            logger.warning("Error in training step - early termination: "+str(e))
                            self.should_training_stop = True
                            self.should_epoch_stop = True
                            break
                else:
